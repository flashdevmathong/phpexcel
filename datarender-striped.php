...
$con = mysqli_connect("localhost","root","","snperp2");
if(!$con){
...
//populate the data
$query = mysqli_query($con,"select * from snpuser");
$row = 4;

$colorState="odd";
while($data = mysqli_fetch_object($query)){
	$excel->getActiveSheet()
		->setCellValue('A'.$row , $data->mid)
		->setCellValue('B'.$row , $data->surname)
		->setCellValue('C'.$row , $data->email)
		->setCellValue('D'.$row , $data->tel);
	//increment the row
	$row++;
